...
class PaiementRecord(models.Model):
    ...
    @api.depends('paiement_lines.amount', 'invoice_ids.amount_residual', 'invoice_ids.move_type')
    def _calc_amount(self):
        for rec in self:
            rec.amount = sum(line.amount for line in rec.paiement_lines)
            rec.amount_invoices = sum(invoice.amount_residual for invoice in rec.invoice_ids if invoice.move_type == 'out_invoice')
            rec.amount_avoirs = sum(avoir.amount_residual for avoir in rec.invoice_ids if avoir.move_type == 'out_refund')
        ...
        for rec in self:
            ...
            for inv in rec.invoice_ids:
                for move_line in inv.move_id.line_ids:
                    if move_line.account_id.move_type == 'receivable':
