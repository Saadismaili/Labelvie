...
class TestAccountAsset(common.TransactionCase):
    ...
    def test_00_account_asset_asset(self):
        ...
        self.browse_ref("os_account_asset.account_asset_asset_vehicles_test0").validate()

        # I check Asset is now in Open state.
        self.assertEqual(self.browse_ref("os_account_asset.account_asset_asset_vehicles_test0").state, 'open',
            'Asset should be in Open state')

        # I compute depreciation lines for asset of CEOs Car.
        self.browse_ref("os_account_asset.account_asset_asset_vehicles_test0").compute_depreciation_board()
        value = self.browse_ref("os_account_asset.account_asset_asset_vehicles_test0")
        ...
        ids = self.env['account.asset.depreciation.line'].search([('asset_id', '=', self.ref('os_account_asset.account_asset_asset_vehicles_test0'))])
        for line in ids:
            line.create_move()

        # I check the move line is created.
        asset = self.env['account.asset.asset'].browse([self.ref("os_account_asset.account_asset_asset_vehicles_test0")])[0]
        ...
        self.assertEqual(self.browse_ref("os_account_asset.account_asset_asset_vehicles_test0").state, 'close',
            'State of asset should be close')

        # WIZARD
        # I create a record to change the duration of asset for calculating depreciation.
        account_asset_asset_office0 = self.browse_ref('os_account_asset.account_asset_asset_office_test0')
        ...
        context = {
            "active_ids": [self.ref("os_account_asset.menu_asset_depreciation_confirmation_wizard")],
            "active_id": self.ref('os_account_asset.menu_asset_depreciation_confirmation_wizard'),
            'type': 'sale'
        }
