...
class DateRange(models.Model):
    ...
    def create_period(self, interval=1):
        date_range_type = self.env['date.range.type'].create({'name': 'Fiscal Period','allow_overlap': False,'fiscal_period': True,})
        period_obj = self.env['date.range'].search([('company_id','=',self.env.company.id)])
        for fy in self:
